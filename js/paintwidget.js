    ...
    Paints.showDescription = function (title, description){
        $('#paint-description-dialog').html(description);
        dialog = $('#paint-description-dialog').dialog({
	...
	Paints.showList = function (data) {
        ...
        if(typeof data.heads != 'undefined' && data.heads.length > 0) {
            var $items  = '<div style="clear: both;">Поддерживаемые головки: <br />';
            for (var i in data.heads) {
                var head = data.heads[i];
                var $item = '<div>' +
		...
		if(typeof data.paints != 'undefined') {
			for (var i in data.paints) {
				...
				var $item = $('<div style="clear: both;">' +
                    ...
                    ($.trim(paint.description) != '' ? ' <a href="#" onclick="Paints.showDescription(\''+paint.brand + ' / Class ' + paint.class.toUpperCase()+'\', \''+paint.description.replace(/([^>\r\n]?)(\r\n|\n\r|\r|\n)/g, '$1<br/>')+'\'); return false;">подробнее...</a>' : '')+
                        '</div>' +
					        '</div>');
				var j = 0;
				for (var index in data.colors) {
					...
					if(paint[color]) {
						var width = (Paints.listWidth / 4) - (margin + padding) * 2;
		...
		$container.hide()
			.html("<p><a href='#' onclick='Paints.resetSearch(); return false;' class='button'>Сброс</a></p>")
			.append($list)
			.fadeIn();
	}
